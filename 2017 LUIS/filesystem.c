...
int mkFS(long deviceSize)
{
	...
	*/
	...
	{
		perror("Error [A6]: filesystem must be contained in disks between 50 KiB & 100 KiB");
		return -1;
	}
	// generate SB values
...
int createFile(char *fileName)
{
	...
	*/
	...
	{
		if (strcmp(SB.inodeArray[newPosition].name, fileName) == 0 && SB.inodeMap[newPosition] == 1) {
	...
	strcpy(SB.inodeArray[newPosition].name, fileName);	
...
{
	// we search for the block position in the inode array wich has the same name as the fileName
	uint8_t position = 0;
	for (position = 0; position < SB.inodeNum; ++position)
	{
		// if the inode.name and the fileName are the same, we obliterate the block
		if (strcmp(SB.inodeArray[position].name, fileName) == 0) {
...
int openFile(char *fileName)
{
	...
	for (aux = 0; aux < SB.inodeNum; aux++) {
		if (strcmp(SB.inodeArray[aux].name, fileName) == 0 && SB.inodeMap[aux] == 1){
			encontrado = 1;
			break;
		}
	}
...
int checkFS(void)
{
	...
	for (position = 1; position < SB.inodeNum; ++position)
	{
		...
		{			
			...
			// if it's okay, we keep looking for errors, if not: we notify it and return -1
			if (SB.inodeArray[position].crc == crcValue)
			{
				printf("checkFS: check passed for the file %s\n", SB.inodeArray[position].name);
			}else{
				perror("checkFS: file corrupted");
				return -1;
			}
		}
	}
